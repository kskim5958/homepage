SELECT 
    ...
    COUNT(IF(status = 4, 1, NULL)) as big_cancel,
    COUNT(IF(status = 5, 1, NULL)) as big_introduction,
    ...
    SUM(CASE WHEN status = 1 THEN cost ELSE 0 END) as big_qutcall_cost,
    ...
    SUM(CASE WHEN status = 4 THEN cost ELSE 0 END) as big_cancel_cost,
    SUM(CASE WHEN status = 5 THEN cost ELSE 0 END) as big_introduction_cost,
